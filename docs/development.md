...
```mermaid
flowchart LR
    S["起動グループと現地前日を決定"] --> A["日程を確認"] --> B["順位を取得<br>内容を確認"]
    B --> C["投稿文を作る"] --> D["Xへ送信"]
```
...
| グループ | 代表タイムゾーン | 投稿対象 |
| --- | --- | --- |
| East | America/New_York | AL East・NL East |
| Central | America/Chicago | AL Central・NL Central |
| West | America/Los_Angeles | AL West・NL West。表示日が8月以降はAL/NLワイルドカードも追加 |

有効化後は代表現地時刻の毎朝8時に起動する。夏時間はSchedulerとタイムゾーン変換に任せる。
表示日は代表現地時刻の前日で、順位取得年もその年を使う。西部の8月1日朝は7月31日表示のためWCなし、8月2日朝からWCを含める。
表示日はシーズン終了判定にも使い、最終日の翌朝は最終日分を投稿する。

取得するのは起動時点の最新順位であり、前日全試合終了時点の順位を保証しない。終了待ち・反映照合・再確認・過去順位の復元は行わない。試合なし・全延期でも正常な順位があれば投稿する。
対象地区を絞る前に全30球団を検証し、WCも全地区から計算する。文面・タグ・順位計算は従来どおり。
