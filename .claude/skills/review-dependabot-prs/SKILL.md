...
4. **バージョン差分に応じた確認**
   ...
   - major: PR本文のリリースノート（`<details>` 内）に加えて upstream のリリースノート・CHANGELOG・移行ガイドを読み、破壊的変更（削除・改名されたAPI、既定値や挙動の変更、対応ランタイムの変更）を列挙する。各項目についてこのリポジトリでの使用箇所を `grep -rn` で探し、1つも該当しなければOK。該当があれば、その変更後の挙動がこのリポジトリで問題ないかまで調べて判断する（コメントに該当箇所と判断理由を書く）。判断に必要な情報が得られない（リリースノートが無い・破壊的変更の内容が不明）場合はNG。
   - 本番コードの依存（配布物・実行環境に含まれるパッケージ）の major は、上記に加えて実行時の挙動変更（既定値・シリアライズ形式・タイムアウト・ログ出力など）も確認対象にする。実行環境が使えるなら PR ブランチをチェックアウトしてビルド・テストを実行し、CI と同じ結果になることを確かめる。リポジトリの CLAUDE.md にこれと異なる方針が書かれていればそちらを優先する。
