...
```mermaid
flowchart LR
    subgraph tf["Terraform管理（modules/scheduled_lambda）"]
        ...
        L --> ROLE["aws_iam_role SuLambdaRole"]
        L -.-> LG["aws_cloudwatch_log_group<br>/aws/lambda/TwitterMlbBot"]
    end
...
1. **ロググループの保持期間設定**（現状無期限 → 30〜90日。`log_retention_days` を変えるだけ）
2. **ツイート全滅を通知するCloudWatchアラーム**（Lambda `Errors` メトリクス + SNS→メール。全件失敗時はLambdaがエラー終了する仕様のため検知可能）
3. **Lambdaタイムアウト60秒への引き上げ**（リトライ導入とセット。[docs/code-improvements.md](../docs/code-improvements.md) 項目1）
4. **デプロイのOIDC化**（GitHub OIDCプロバイダー + デプロイ用IAMロールをTerraformで作成。[docs/developer-experience.md](../docs/developer-experience.md) 項目1）
5. **実行ロールの最小権限化**（`CloudWatchFullAccess` は過剰。ログ書き込みのみに絞る）
6. **Terraform実行用IAMロールの新設（AssumeRole方式）** … 長期キーを増やさずに実行主体を分離し、CloudTrailで操作を追跡可能にする。OIDC化（候補4）と同じロールに統合すると管理が一元化できる
7. **APIキーをSSM Parameter Store（SecureString・無料）へ移行** … アプリが起動時に `ssm:GetParametersByPath` で読む方式にすると、Lambda環境変数とtfstateから機密が消え、キー更新もCLIで完結する（`environment` のignore_changesも不要になる）。実行ロールへの権限付与はTerraform、パラメータ登録はCLI、読み込みはアプリ側の対応
